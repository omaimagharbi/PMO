...
3. **Définir vos secrets d'authentification** dans `.env` : `JWT_SECRET` (une chaîne aléatoire longue, ex: générée avec `openssl rand -hex 32`). ⚠️ `ADMIN_PASSWORD` n'est **plus utilisé** depuis la mise en place des comptes multi-utilisateurs (voir section "Comptes et rôles" ci-dessous) — vous pouvez retirer cette variable de vos Environment Variables Vercel.
...
5. **Créer votre premier projet et récupérer son ID** :
   ...
   Copiez l'UUID de "AtlasApp" (créé automatiquement par `schema.sql`) et collez-le dans le champ **"ID Projet"** en haut de l'interface, ou utilisez le sélecteur de projet une fois connecté.
...
7. **Déclarer votre équipe (Scénario 2)** — depuis l'onglet 05 "Gestion" une fois connecté, ou en SQL :
...
10. **Créer votre compte admin** (une seule fois) :

    ```bash
    node scripts/create-user.js --email vous@catalyste.tn --password "VotreMotDePasseFort" --role admin
    ```

    Puis connectez-vous dans l'interface avec cet email et ce mot de passe. Le jeton obtenu n'est valable que 12h et n'est pas persisté au rechargement de la page (reconnectez-vous si nécessaire).

## Comptes et rôles

Depuis la migration `003_multi_tenant_foundations.sql`, NEXUS gère deux rôles :

- **`admin`** (vous, Catalyste) : accès à tous les projets et à tous les onglets, y compris 02 (portefeuille cross-projets), 03 (mémoire organisationnelle cross-projets) et 05 (Gestion).
- **`client`** : accès restreint au(x) projet(s) qui lui ont été accordés. Les onglets 02, 03 et 05 sont masqués et refusés côté API pour ce rôle, car ils exposent des données transverses à plusieurs clients.

Pour créer un compte client, utilisez le formulaire "Créer un compte client" de l'onglet 05 (recommandé), ou en ligne de commande :

```bash
node scripts/create-user.js --email client@exemple.tn --password "MotDePasseTemporaire" --role client --project <UUID_PROJET> --name "Nom du contact"
```

Communiquez le mot de passe temporaire au client par un canal séparé et sécurisé (jamais par le même message que l'email/identifiant).
