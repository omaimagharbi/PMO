...
async function handleSnapshotGet(req, res, user) {
...
async function handleSnapshotPost(req, res, user) {
...
async function handleUpdateTask(req, res, user) {
  ...
  try {
    const taskResult = await query(`SELECT project_id FROM project_tasks WHERE id = $1`, [taskId]);
    if (taskResult.rows.length === 0) {
      res.status(404).json({ error: 'Tâche introuvable.' });
      return;
    }

    if (!(await requireProjectAccess(req, res, user, taskResult.rows[0].project_id))) return;
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET' && action === 'snapshot') return handleSnapshotGet(req, res, user);
  if (req.method === 'POST' && action === 'snapshot') return handleSnapshotPost(req, res, user);
  if (req.method === 'POST' && action === 'update-task') return handleUpdateTask(req, res, user);
