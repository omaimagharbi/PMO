...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const repoFullName = payload.repository && payload.repository.full_name ? payload.repository.full_name : null;
    let resolvedProjectId = null;
    if (repoFullName) {
      const projectResult = await query(`SELECT id FROM projects WHERE github_repo_full_name = $1`, [repoFullName]);
    ...
    await query(
      ...
      [resourceId, resolvedProjectId, externalEventId, hoursLogged, eventDate, JSON.stringify(payload)]
    );

    res.status(200).json({ received: true, resourceId, hoursLogged, prNumber: pr.number, resolvedProjectId });
  } catch (err) {
