...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (projectKey) {
      const projectResult = await query(`SELECT id FROM projects WHERE jira_project_key = $1`, [projectKey]);
    ...
    res.status(200).json({ received: true, resourceId, hoursLogged, projectKey, resolvedProjectId });
  } catch (err) {
