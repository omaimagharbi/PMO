...
module.exports = async function handler(req, res) {
  try {
    ...
    if (uploadedFile.mimetype !== 'application/pdf') {
      res.status(400).json({
        error: 'Seuls les fichiers PDF sont acceptés pour la charte de projet (type détecté : ' + (uploadedFile.mimetype || 'inconnu') + ').'
      });
      return;
    }
    ...
    // Filet de sécurité : garantit une réponse JSON même en cas d'erreur
    // totalement imprévue (ex: dépendance manquante, crash interne), pour
    // ne jamais renvoyer la page HTML générique de Vercel au frontend.
    if (!res.headersSent) {
      res.status(500).json({ error: 'Erreur interne inattendue : ' + err.message });
    }
  }
};
