(function () {
  ...
  var authToken = null;
  var currentUser = null; // { id, email, role, fullName }
  ...
  async function login() {
    ...
    if (!email || !password) {
      setAuthStatus('Saisissez votre email et votre mot de passe.', true);
      return;
    }
    ...
    try {
      ...
      setAuthStatus('✓ Connecté en tant que ' + data.user.email + ' (' + data.user.role + ').', false);
      applyRoleVisibility();
      ...
      if (typeof refreshProjectSelector === 'function') {
  ...
  function initConfigBar() {
    ...
    document.getElementById('refreshProjectSelectorBtn').addEventListener('click', refreshProjectSelector);
  }

  async function refreshProjectSelector() {
    var select = document.getElementById('projectSelector');
    try {
      var response = await fetch('/api/admin/actions?action=list-projects', { headers: authHeaders() });
      var data = await response.json();
      ...
      /* liste non chargée, on laisse le champ UUID manuel disponible */
    }
  }
  ...
  /* ============================================================
     ONGLET 05 — GESTION (créer projet / équipe / baseline sans SQL)
  ============================================================ */
  ...
  async function createProject() {
    ...
    var rate = document.getElementById('newProjectRate').value;

    if (!name) {
      setStatus('createProjectStatus', 'Le nom du projet est requis.', true);
      return;
    }

    try {
      var response = await fetch('/api/admin/actions?action=create-project', {
        method: 'POST',
        headers: authHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ name: name, dailyRateEur: rate || undefined })
      });
      var data = await response.json();

      if (!response.ok) {
        setStatus('createProjectStatus', 'Erreur : ' + data.error, true);
        return;
      }

      setStatus('createProjectStatus', '✓ Projet "' + data.project.name + '" créé. ID : ' + data.project.id, false);
      document.getElementById('newProjectName').value = '';
      document.getElementById('newProjectRate').value = '';
      refreshProjectSelector();
    } catch (err) {
      setStatus('createProjectStatus', 'Erreur réseau : ' + err.message, true);
    }
  }

  async function saveIntegrations() {
    var projectId = getProjectId();
    var jiraKey = document.getElementById('integJiraKey').value.trim();
    var githubRepo = document.getElementById('integGithubRepo').value.trim();

    if (!projectId) {
      setStatus('integrationsStatus', "Sélectionne d'abord un projet en haut de page.", true);
      return;
    }

    try {
      var response = await fetch('/api/admin/actions?action=update-integrations', {
        method: 'POST',
        headers: authHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ projectId: projectId, jiraProjectKey: jiraKey || undefined, githubRepoFullName: githubRepo || undefined })
      });
      var data = await response.json();

      if (!response.ok) {
        setStatus('integrationsStatus', 'Erreur : ' + data.error, true);
        return;
      }

      setStatus('integrationsStatus', '✓ Clés enregistrées pour "' + data.project.name + '".', false);
    } catch (err) {
      setStatus('integrationsStatus', 'Erreur réseau : ' + err.message, true);
    }
  }

  async function refreshAssignResourceSelect() {
    var select = document.getElementById('assignResourceSelect');
    try {
      var response = await fetch('/api/admin/actions?action=list-resources', { headers: authHeaders() });
      var data = await response.json();
      if (!response.ok) return;
      ...
      data.resources.forEach(function (r) {
        var opt = document.createElement('option');
        opt.value = r.id;
        opt.textContent = r.full_name + ' (' + r.role + ')';
        select.appendChild(opt);
      });
    } catch (err) {
      /* liste non chargée */
  ...
  async function createResource() {
    var fullName = document.getElementById('newResourceName').value.trim();
    var role = document.getElementById('newResourceRole').value.trim();
    var capacity = document.getElementById('newResourceCapacity').value;
    var jiraId = document.getElementById('newResourceJira').value.trim();
    var githubUser = document.getElementById('newResourceGithub').value.trim();

    if (!fullName || !role) {
      setStatus('createResourceStatus', 'Le nom et le rôle sont requis.', true);
      return;
    }

    try {
      var response = await fetch('/api/admin/actions?action=create-resource', {
        method: 'POST',
        headers: authHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({
          fullName: fullName, role: role,
          weeklyCapacityHours: capacity || undefined,
          jiraAccountId: jiraId || undefined,
      ...
      if (!response.ok) {
        setStatus('createResourceStatus', 'Erreur : ' + data.error, true);
        return;
      }

      setStatus('createResourceStatus', '✓ Ressource "' + data.resource.full_name + '" créée.', false);
      document.getElementById('newResourceName').value = '';
      document.getElementById('newResourceRole').value = '';
      document.getElementById('newResourceCapacity').value = '';
      document.getElementById('newResourceJira').value = '';
      document.getElementById('newResourceGithub').value = '';
      refreshAssignResourceSelect();
    } catch (err) {
      setStatus('createResourceStatus', 'Erreur réseau : ' + err.message, true);
    }
  }

  async function assignResource() {
    var projectId = getProjectId();
    var resourceId = document.getElementById('assignResourceSelect').value;
    var allocationPct = document.getElementById('assignAllocationPct').value;

    if (!projectId) {
      setStatus('assignResourceStatus', "Sélectionne d'abord un projet en haut de page.", true);
      return;
    }
    if (!resourceId) {
      setStatus('assignResourceStatus', 'Choisis une ressource dans la liste (rafraîchis si la liste est vide).', true);
      return;
    }

    try {
      var response = await fetch('/api/admin/actions?action=assign-resource', {
        method: 'POST',
        headers: authHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ projectId: projectId, resourceId: resourceId, allocationPct: allocationPct || undefined })
      });
      var data = await response.json();

      if (!response.ok) {
        setStatus('assignResourceStatus', 'Erreur : ' + data.error, true);
        return;
      }

      setStatus('assignResourceStatus', '✓ Ressource affectée à ' + data.assignment.allocation_pct + '%.', false);
    } catch (err) {
      setStatus('assignResourceStatus', 'Erreur réseau : ' + err.message, true);
    }
  }
  ...
  async function createTask() {
    ...
    var startDay = document.getElementById('newTaskStartDay').value;
    var duration = document.getElementById('newTaskDuration').value;
    var cost = document.getElementById('newTaskCost').value;
    var pct = document.getElementById('newTaskPct').value;

    if (!projectId) {
      setStatus('createTaskStatus', "Sélectionne d'abord un projet en haut de page.", true);
      return;
    }
    if (!name || startDay === '' || duration === '' || cost === '') {
      setStatus('createTaskStatus', 'Le nom, le jour de début, la durée et le coût sont requis.', true);
      return;
    }

    try {
      var response = await fetch('/api/admin/actions?action=create-task', {
        method: 'POST',
        headers: authHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({
          projectId: projectId, name: name,
          plannedStartDay: startDay, plannedDurationDays: duration,
          plannedCostEur: cost, pctComplete: pct || undefined
        })
      });
      var data = await response.json();

      if (!response.ok) {
        setStatus('createTaskStatus', 'Erreur : ' + data.error, true);
        return;
      }

      setStatus('createTaskStatus', '✓ Tâche "' + data.task.name + '" ajoutée à la baseline.', false);
      document.getElementById('newTaskName').value = '';
      document.getElementById('newTaskStartDay').value = '';
      document.getElementById('newTaskDuration').value = '';
      document.getElementById('newTaskCost').value = '';
      document.getElementById('newTaskPct').value = '';
    } catch (err) {
      setStatus('createTaskStatus', 'Erreur réseau : ' + err.message, true);
    }
  }

  async function setStartDate() {
    var projectId = getProjectId();
    var startedOn = document.getElementById('newStartDate').value;

    if (!projectId) {
      setStatus('setStartDateStatus', "Sélectionne d'abord un projet en haut de page.", true);
      return;
    }
    if (!startedOn) {
      setStatus('setStartDateStatus', 'Choisis une date.', true);
      return;
    }

    try {
      var response = await fetch('/api/admin/actions?action=set-start-date', {
        method: 'POST',
        headers: authHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ projectId: projectId, startedOn: startedOn })
      });
      var data = await response.json();

      if (!response.ok) {
        setStatus('setStartDateStatus', 'Erreur : ' + data.error, true);
        return;
      }

      setStatus('setStartDateStatus', '✓ Date de démarrage enregistrée : ' + data.startDate.started_on, false);
    } catch (err) {
      setStatus('setStartDateStatus', 'Erreur réseau : ' + err.message, true);
    }
  }
  ...
  function initScenario5() {
    ...
    document.getElementById('createResourceBtn').addEventListener('click', createResource);
